        ...
        <div className="w-full flex flex-col justify-center items-start rounded-2xl overflow-hidden bg-(--clr-surface)">
            ...
            <div className="w-full overflow-x-auto">
                <div className="min-w-fit w-full flex justify-between items-center text-gray-400 bg-(--clr-surface2) border border-(--clr-surface-light2) font-bold p-4">
                    <div className="flex justify-center items-center gap-1 w-full">
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-full lg:max-w-37.5 text-center">User name</p>
                        <p className="min-w-60 lg:min-w-auto max-w-60 lg:w-full lg:max-w-1/4 text-center">Email</p>
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-30 text-center">Status</p>
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-25 text-center">Type</p>
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-25 text-center">Spendings</p>
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-full lg:max-w-25 text-center">Services</p>
                        <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-full lg:max-w-25 text-center">Actions</p>
                    </div>
                </div>
                {users.map((u) => (
                    <div key={u.full_name} className={`min-w-fit w-full flex justify-between items-center text-gray-400 border border-(--clr-surface-light2) hover:bg-(--clr-surface-light2) p-4 transition`}>
                        <div className="flex justify-center items-center gap-1 w-full">
                            <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-full lg:max-w-37.5 text-[14px] text-center font-bold">{u.full_name}</p>
                            <p className="min-w-60 lg:min-w-auto max-w-60 lg:w-full lg:max-w-1/4 text-center text-[14px]">{u.email}</p>
                            <p className={`min-w-40 lg:min-w-auto max-w-40 text-center p-2 rounded-xl cursor-pointer lg:w-30 font-bold ${u.verified ? "text-green-400 bg-green-400/20" : "text-yellow-400 bg-yellow-400/20"}`}>
                                {u.verified ? "Verified" : "Not verified"}
                            </p>
                            <button
                                onClick={() => {}}
                                className={`min-w-40 lg:min-w-auto max-w-40 lg:w-25 text-center font-bold p-2 rounded-xl cursor-pointer ${u.admin ? "text-green-400 bg-green-400/20" : "text-orange-400 bg-orange-400/20"}`}
                            >
                                {u.admin ? "Admin" : "User"}
                            </button>
                            <p className="min-w-40 lg:min-w-auto max-w-40 lg:w-25 text-center font-bold">${u.total_spent}</p>
                            <div className="min-w-40 lg:min-w-auto max-w-40 lg:w-full lg:max-w-25 text-center">
                            ...
                            <Button
                                ...
                                css="danger lg:w-full lg:max-w-25 min-w-40 lg:min-w-auto max-w-40"
                                action={async () => {
            ...
            {sub_view && (
                <div ref={dark_bg} className="fixed w-dvw h-dvh left-1/2 top-1/2 -translate-1/2 bg-black/75 z-50 flex justify-center items-center" onClick={(e) => e.target == dark_bg.current && set_sub_view(null)}>
                    <Button content="Close" css="absolute right-4 top-4 lg:top-1/8 bg_gray" action={() => set_sub_view(null)} />

                    <div className="w-[95dvw] h-4/5 lg:min-w-200 lg:w-[50dvw] lg:h-[80dvh] lg:mt-[10dvh] p-4 rounded-2xl bg-(--clr-surface) border-2 border-(--clr-surface-light2) flex justify-center items-center flex-col gap-4">
                        <h1 className="text-2xl font-bold text_shine w-full text-center">{sub_view.username}&apos;s services</h1>
                        <div className="flex justify-start items-center flex-col gap-4 w-full max-h-[90%] overflow-x-hidden overflow-y-auto">
                            {sub_view.subs.map((s) => {
...
function Service({ s, email }: { s: sub; email: string }) {
    ...
    return (
        <div className="rounded-2xl w-full font-bold text-gray-400 flex flex-col justify-center items-start gap-8 lg:gap-2">
            ...
            <div className="w-full  flex justify-between items-center gap-4 px-4 flex-wrap">
                ...
                <div className="w-[calc(50%-8px)]  bg-(--clr-surface-light2) rounded-2xl flex flex-col justify-center items-start p-4 gap-2">
                    <p>Purchase Date</p>
                    <p className="text-white lg:text-xl">{start_date.getFullYear() + "-" + (start_date.getMonth() + 1) + "-" + start_date.getDate()}</p>
                </div>
                <div className="w-[calc(50%-8px)]  bg-(--clr-surface-light2) rounded-2xl flex flex-col justify-center items-start p-4 gap-2">
                    <p>Ending Date</p>
                    {edit ? (
                        ...
                        <p className="text-white lg:text-xl">{exp_date.getFullYear() + "-" + (exp_date.getMonth() + 1) + "-" + exp_date.getDate()}</p>
                    )}
                </div>
            </div>
            ...
            <div className="text-xl w-full flex justify-center items-center flex-wrap px-4 text-white">
                <div className="w-full flex justify-between items-center">
                    <p className="min-w-fit">Time left</p>
                    <div className="min-w-fit text-end">{calcTime(now, exp_date)}</div>
                </div>
