				...
				<Board
					...
					content={
						<>
							...
							<div className={`flex justify-center items-center w-full min-h-16 bg-(--clr-surface2) p-4 border border-(--clr-surface-light2) hover:bg-(--clr-surface) transition loading`}></div>
							<div className={`flex justify-center items-center w-full min-h-16 bg-(--clr-surface2) p-4 border border-(--clr-surface-light2) hover:bg-(--clr-surface) transition loading`}></div>
							<div className={`flex justify-center items-center w-full min-h-16 bg-(--clr-surface2) p-4 border border-(--clr-surface-light2) hover:bg-(--clr-surface) transition loading rounded-b-2xl`}></div>
						</>
					}
				/>
			</div>
			...
			{bills.length ? (
				<Board
					...
					content={bills.map((t, i) => (
						<div key={t.description + i} className={`flex justify-center items-center w-full min-h-8 bg-(--clr-surface2) p-4 border border-(--clr-surface-light2) hover:bg-(--clr-surface) transition ${i + 1 == bills.length && "rounded-b-2xl"}`}>
							<p className="font-bold text-gray-400 w-1/5">{t.date}</p>
							<p className="font-bold text-gray-400 w-2/5">{t.description}</p>
							<div className={`font-bold text-xs  w-1/5 ${t.paid ? "text-green-400" : "text-red-400"}`}>
							...
							<p className="font-bold text-gray-400 w-1/5">${t.price}</p>
						</div>
					))}
				/>
			) : (
