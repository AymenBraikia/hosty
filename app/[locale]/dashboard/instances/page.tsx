            ...
            {instances.length ? (
                <Board
                    ...
                    content={instances.map((instance, i) => (
                        <div
                            key={instance.ip}
                            className={`min-w-fit flex justify-center items-center w-full min-h-8 bg-(--clr-surface2) p-4 border border-(--clr-surface-light2) hover:bg-(--clr-surface) transition ${i + 1 == instances.length && "rounded-b-2xl"}`}
                        >
                            <p className="min-w-40 lg:min-w-auto font-bold text-gray-400 w-full">{instance.name}</p>
                            <div className="font-bold text-gray-400 w-full">
                                {instance.status ? (
                                    <div className="min-w-40 lg:min-w-auto flex justify-start items-center gap-2 text-green-400">
                                        <div className="rounded-full w-2 pulse aspect-square bg-current"></div>
                                        <div className="text-current">Running</div>
                                    </div>
                                ) : (
                                    <div className="min-w-40 lg:min-w-auto flex justify-start items-center gap-2 text-red-400">
                                        <div className="rounded-full w-2 pulse aspect-square bg-current"></div>
                                        <div className="text-current">Stopped</div>
                                    </div>
                                )}
                            </div>
                            <p className="min-w-40 lg:min-w-auto font-bold text-gray-400 w-full">{instance.ip}</p>
                            <p className="min-w-40 lg:min-w-auto font-bold text-gray-300 w-full">{instance.cpu}</p>
                            <ActionBtns />
                        </div>
                    ))}
                />
