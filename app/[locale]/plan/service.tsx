    ...
    return data ? (
        <div className="flex justify-center items-start w-full gap-16">
            <div className="bg-(--clr-surface) relative overflow-hidden p-16 rounded-2xl border border-(--clr-surface-light2) flex flex-col justify-center items-start gap-8 w-1/2">
                ...
                <AtcBtn available={user_info!.cart.find((e) => typeof e.id == "number" && e.id == data.id) ? false : true} product_id={data.id} />
                <AddWishBtn available={[...user_info!.wish_list, ...user_info!.cart].find((e) => typeof e.id == "number" && e.id == data.id) ? false : true} product_id={data.id} />
            </div>
