...
export default function Select(props: { default?: string; action: (e: React.MouseEvent) => void; options: string[][]; css?: string }) {
    ...
    return (
        <div onClick={() => set_active(!is_active)} className={`relative lg:text-[16px] text-xl outline-0 cursor-pointer hover:rounded-none z-50 ${props.css}`}>
            {props.default || props.options[0][1] || props.options[0][0]}

            {is_active && (
                <div className="min-w-40 lg:min-w-auto max-h-40 bg-(--clr-surface) rounded-xl overflow-y-auto no_scrollbar absolute -bottom-2.5 -left-4 translate-y-full">
