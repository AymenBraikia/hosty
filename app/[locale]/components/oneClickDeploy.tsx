		...
		<section className="min-h-dvh w-dvw px-[10dvw] flex max-md:p-7 justify-center items-center text-center">
			<div className="min-h-1/2 w-full bg-(--clr-surface) flex max-md:flex-col justify-center items-center p-10 relative rounded-2xl border-2 border-(--clr-surface-light2) overflow-hidden max-md:gap-10">
				<div className="flex flex-col justify-center items-center gap-7 p-11 w-1/2 max-md:w-full max-md:p-0 z-10">
					<h2 className="text-4xl font-black max-sm:text-2xl">{t("headline")}</h2>
					<p className="text_shine text-xl text-start">{t("description")}</p>
				...
				<div className="flex flex-wrap justify-center items-center gap-7 w-1/2 max-md:w-full select-none z-10">
					{apps.map((app, i) => {
						return (
							<div
								key={i}
								className="w-[27%] max-sm:w-5/12 max-sm:max-w-32 rounded-2xl aspect-square text-gray-400 bg-(--clr-surface2) cursor-pointer transition border-2 border-(--clr-surface) hover:border-gray-700 flex justify-center items-center flex-col gap-2"
							>
								{app.icon}
								{app.title}
							</div>
