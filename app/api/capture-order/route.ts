...
import { domainSub, hostServiceSub, order } from "@/app/[locale]/types/product";
...
export async function POST(req: Request) {
    ...
    const computes = order.items.filter((e) => typeof e.id == "number");
    const domains = order.items.filter((e) => typeof e.id == "undefined");
                    ...
                    $each: computes.map(
                        (e) =>
                            ({
                                ...
                                suspended: false,
                                started_at: new Date(),
                                expire_at: new Date(new Date().setMonth(new Date().getMonth() + e.amount)),
                            }) as hostServiceSub,
                    ),
                },
                "services.domains": {
                    $each: domains.map(
                        (e) =>
                            ({
                                ...
                                suspended: false,
                                started_at: new Date(),
                                expire_at: new Date(new Date().setFullYear(new Date().getFullYear() + e.years)),
                            }) as domainSub,
                    ),
                },
