...
import { Bill, domainSub, hostServiceSub, domainOrder, hostServiceOrder } from "@/app/[locale]/types/product";
...
export async function POST() {
    ...
    const cart = [...user.cart.compute, ...user.cart.domains];
    const first_purchase = user.first_purchase as boolean;

    const items = cart.map((item) => {
        return {
            name: item.type,
            description: "name" in item ? `Domain Registration for ${item.name}` : item.description,
    ...
    await ordersCollection.insertOne({
        ...
        items: cart.map((i) => {
            ...
            return "name" in i
                ? ({ ...i, started_at, expire_at, renew: true, role: "owner", years: i.amount, extension: i.name!, active: true, name: i.name! } as domainSub)
                : ({ ...i, started_at, expire_at, renew: true, role: "owner", active: true } as hostServiceSub);
        }),
        total_amount,
        vat_amount,
        discount_amount,
