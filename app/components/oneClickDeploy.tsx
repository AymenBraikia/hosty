...
export default function OneClickDeploy() {
	return (
		<div className="h-dvh w-dvw px-[10dvw] flex justify-center items-center">
			<div className="min-h-1/2 w-full bg-(--clr-surface) flex justify-between items-center p-10 relative rounded-2xl border-2 border-(--clr-surface-light2) overflow-hidden">
				<div className="flex flex-col justify-center items-start gap-7 p-11 w-1/2 z-10">
					<h2 className="text-4xl font-black">
						Deploy your favorite apps with <span className="text_anim2">1-Click</span>
					</h2>
					<p className="text_shine text-xl">Forget complex server configurations. Hosty&apos;s marketplace lets you launch production-ready instances of popular software instantly.</p>
				...
				<div className="flex flex-wrap justify-center items-center gap-7 w-1/2 select-none z-10">
					{apps.map((app, i) => {
						return (
							<div key={i} className="w-[27%] rounded-2xl aspect-square text-gray-400 bg-(--clr-surface2) cursor-pointer transition border-2 border-(--clr-surface) hover:border-gray-700 flex justify-center items-center flex-col gap-2">
								{app.icon}
								{app.title}
							</div>
						);
